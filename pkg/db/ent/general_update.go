...
func (gu *GeneralUpdate) Modify(modifiers ...func(u *sql.UpdateBuilder)) *GeneralUpdate {
...
func (guo *GeneralUpdateOne) Save(ctx context.Context) (*General, error) {
	...
	if len(guo.hooks) == 0 {
		...
		nv, ok := v.(*General)
		if !ok {
			return nil, fmt.Errorf("unexpected node type %T returned from GeneralMutation", v)
		}
		node = nv
	}
	return node, err
...
// Modify adds a statement modifier for attaching custom logic to the UPDATE statement.
func (guo *GeneralUpdateOne) Modify(modifiers ...func(u *sql.UpdateBuilder)) *GeneralUpdateOne {
