...
func trace(span trace1.Span, in *npool.DetailReq, index int) trace1.Span {
	span.SetAttributes(
		...
		attribute.Float64(fmt.Sprintf("Amount.%v", index), in.GetAmount()),
		attribute.String(fmt.Sprintf("FromCoinTypeID.%v", index), in.GetFromCoinTypeID()),
		attribute.Float64(fmt.Sprintf("CoinUSDCurrency.%v", index), in.GetCoinUSDCurrency()),
...
func TraceConds(span trace1.Span, in *npool.Conds) trace1.Span {
	span.SetAttributes(
		...
		attribute.Float64("Amount.Value", in.GetAmount().GetValue()),
		...
		attribute.Float64("CoinUSDCurrency.Value", in.GetCoinUSDCurrency().GetValue()),
